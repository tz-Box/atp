"""pipe.slam 数据 schema：观测 payload（SlamData）与结果（CylinderResult）。"""
...
from autotest.protocol.schema import Imu, Pose, StampedPose, register_data
...
def _encode_slam_obs(obj):
    return msgpack.packb(obj.to_dict(), use_bin_type=True)


def _encode_slam_result(obj):
    return msgpack.packb(obj.to_dict(), use_bin_type=True)


def _decode_obs(b: bytes):
    return SlamData.from_dict(msgpack.unpackb(b, raw=False))


def _decode_result(b: bytes):
    return CylinderResult.from_dict(msgpack.unpackb(b, raw=False))


def _decode_stamped_pose(b: bytes):
    return StampedPose.from_dict(msgpack.unpackb(b, raw=False))


def register_schemas():
    """注册跨进程数据 schema（observation/result）。

    GT（pipe.slam.Trajectory / pipe.slam.PipeSegment）不注册：进程内传递、不编码，
    由 World 以 {schema, v, data} 信封声明、checker 自行解析（v1.1 §6）。
    """
    register_data("observation", "pipe.slam.SlamObs", _decode_obs, _encode_slam_obs)
    register_data("result", "pipe.slam.CylinderResult", _decode_result, _encode_slam_result)
    register_data("result", "pipe.slam.StampedPose", _decode_stamped_pose, _encode_slam_result)
