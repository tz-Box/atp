"""pipe.slam checker：APE / RPE（绝对/相对轨迹误差）与管道中轴线检测。"""
...
from .data import CylinderResult


def _decode_results(records: list[Any], schema: str) -> list[Any]:
    """从开环 records（result payload {module, data} 列表）按 schema 过滤并解码。"""
    return [
        decode_result(rec["data"])
        for rec in records
        if isinstance(rec, dict) and rec.get("data", {}).get("schema") == schema
    ]
...
class PipeChecker(IChecker):
    ...
    def evaluate(self, records: list[Any], ground_truth: dict, config: Optional[dict[str, Any]] = None) -> Score:
        ...
        cylinders = [
            c for c in _decode_results(records, "pipe.slam.CylinderResult")
