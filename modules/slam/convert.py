"""SLAM 数据源公共转换：ROS 消息 → 协议帧（replay 回放 / rostopic 实时共用）。

replay 与 rostopic 只是"帧的来源"不同，消息→Observation 的转换逻辑
...
from autotest.protocol.schema import SLAM, Imu, Observation
...
def _stamp_from_dict(raw: dict) -> float:
    """样本 dict 的时间戳：优先 ts.t_ref_ns（device 层参考钟，见 Patrol Box 对接文档）；
...
class SlamRostopicConverter:
    ...
    def convert(self, topic: str, msg) -> Optional[Observation]:
        ...
        if topic in self._lidar_by_topic:
            ...
            t0, sensors = frame
            return Observation(t0, SLAM, SlamData(sensors=sensors))
        return None
...
class SlamDeviceConverter:
    ...
    def convert(self, topic: str, raw: dict) -> Optional[Observation]:
        if topic in self._imu_by_topic:
