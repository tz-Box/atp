"""SLAM rosbag2 数据源：每个 rosbag2 包为一个 testcase，读 lidar → SlamData。
...
from autotest.protocol.schema import SLAM, Imu, Observation, Pose, StampedPose
...
class RosbagSlamDataset:
        ...
        try:
            ...
            for conn, _ts, raw in reader.messages():
                if conn.topic in imu_by_topic:
                    ...
                    t0, sensors = frame
                    count += 1
                    yield Observation(t0, SLAM, SlamData(sensors=sensors))
        finally:
            reader.close()
    ...
    def ground_truth(self, testcase_id: str) -> GroundTruth:
        ...
        if pipe_seg.is_file():
            return GroundTruth(data={"pipe_segment": read_pipe_segment(pipe_seg)})
        ...
        return GroundTruth.trajectory(trajectory)
