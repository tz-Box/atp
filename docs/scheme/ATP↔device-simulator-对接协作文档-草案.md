...
**ATP 补一条消费侧的理由，指向同一个选项 (a)**：
replay 把 `ts` **原样还原**（§2.1），而 ATP 的**严格回归档正建立在「录制内容及其含义都不变」之上**。
选 (b) 则**旧录制里参考域帧的 `sync_class` 含义与新录制不同，而帧上没有任何版本标记能把两者分开**
—— 严格档的比对基准会静默劈成两半。**这正是 §2 那条纪律要防的形态，且发生在我最依赖的那份存档里。**
...
| **7** | **契约层 `sync_class` 文档与参考域行为矛盾**（`timebase.py:92` 说「本帧实际达到的等级」，`:371-374` 返回声明值）→ **选 (a)：改文档不改行为** | device 提，ATP 复核并加消费侧理由 | **建议选 (a)。** device 的理由（参考域 `t_ref=t_rx` 物理上仍在参考钟上，标 S0 是把「盒子自己的钟」说成「无同步」）+ **ATP 的理由：选 (b) 会让旧录制与新录制里参考域帧的 `sync_class` 含义不同而帧上无版本标记可分，严格回归档的基准静默劈成两半**。(a) 不改任何已录数据语义。与候选 5 独立，互不阻塞 |
