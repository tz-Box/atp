...
### 7.2.1 批准后的执行归属（三方各自认领，已确认）

| 候选 | 谁改 | 改在哪 | 怎么交付 |
|---|---|---|---|
| **1** `stamp_sim_ns` | **simulator** | `feature/dynamics`：`gz_bridge.py` 两处回调 + `sources.py` + schema 一条可选字段 | 独立提交 + 一条测试钉住「等于 header stamp 而非 `/clock`」；**回改动摘要给 ATP 核对，不由 ATP 转述** |
| **5** `else 0` + 两处附带 | **simulator** | `feature/dynamics`：`gz_bridge.py:778` + `degradation.py` + `data_check` | 独立提交 + 测试；同上回摘要 |
| **7** 文档改准 (a) | **device** | 本仓 `timebase.py:92` 那段文档 + 同步 `timestamp.schema.json` 的描述 | 完成后告知 ATP 版本 |
| **6** 分支策略 | 归 M | — | 未决，不阻塞其余三项 |
| — ATP 侧全部 | **ATP** | 见 §7.3 | 依赖 1/5/7 的仅为判据措辞，**其余可即刻开工** |

> **三项互不阻塞**，可并行。ATP 的 §7.3 头三项（`DeviceWorld` 重写、converter 重写、
> 去掉 `time.time()` 回退）**不依赖任何一项批准**，样例已在手。

### 7.2.2 已收口：`stamp_sim_ns` 的边界由谁守（2026-09-11）

字段进两仓**共享**的雷达 schema（device 2 文件），是被「共享 schema 逐字节一致」的
parity 规则逼出来的形态。**谁来防止它被用进 backend 无关的解析路径**，四条路评估如下：

| | 做法 | device 侧 | 防住谁 |
|---|---|---|---|
| **1** | 字段进共享 schema，description 写明「仿真独有、真机不发」 | **2 文件** | 无强制 |
| 2 | simulator 放宽 parity，允许仿真侧扩展字段 | 0 文件 | 无强制；严格规则开口子，例外会长出例外 |
| 3 | 加机器可读标记 `x-only-backends: [...]` + 新写校验 | 3 文件 + 1 条校验 | 所有消费者 |
| **4** | **ATP 在自己 converter 里断言 + 注释钉死** | **0 文件（增量）** | **只防 ATP 自己** |

**结论：路 1 + 路 4。** ⚠ 注意路 4 **不省掉**路 1 的 2 个文件 ——
字段仍要进共享 schema；路 4 的「0」是**增量**不是总量。
（初稿的表把这两个数字混在一列，会读成「选 4 就不用改 device」，已更正。）

**为什么不选路 3**（device 判生产侧那半后收口）：真机五个 adapter 的 `fields` 全由各自
`layout_fields()` 与显式键构成，`grep stamp_sim|t_sim` 在 `src/tz_device_backends/` 为空。
「没有东西保证不会发」今天的唯一成立路径，是**有人把 simulator 的 `GzLidar` 代码抄进真机 adapter**
—— **那是代码评审该拦的，不是 schema 标记能拦的。**

> 这句才是决定性的：**它说明路 3 拦的位置不对，而不只是「现在还不需要」。**
> ATP 原本的倾向（路 3）建立在「生产侧也需要拦」的假设上，该假设不成立。

**ATP 侧路 4 的落法**（不需任何人批准，属 ATP 代码纪律，随 converter 重写一并落）：
`stamp_sim_ns` 只在真值对齐函数内读取，函数上写明**为什么**不能外移 ——
不写「勿动」，写清楚外移后切真机会**静默失去对齐且不报错**。**理由比禁令耐久。**

#### 备选 3-lite（不推，但触发条件写死）

device 提供：不动元 schema、不加标记，只在 L3 一致性测试加一条 ——
`transport.type != sim` 的目标，样本 `fields` 里出现 `stamp_sim_ns` 即失败。
**1 个测试文件**，把「真机不会发」从散文变成红绿；代价是键名硬编码。

**翻出来的时机（两者任一，device 补的第二条比 ATP 原提的更早到）**：
1. 出现**第二个仿真专属键** —— 键名硬编码的代价开始显形，且标记方案届时有两个真实例子可参照，不必凭想象定形状；
2. **出现第二个走 device wire 的消费者**（ATP 之外的 SLAM / 导航进程直接订 DATA 口）
   —— **路 4 只防 ATP 自己，第二个消费者一出现它立刻失效**，那时 3-lite 是最便宜的补位。

> 写死触发条件而不是「以后再说」：本轮已多次见到**「写下来就当处理完了」**这一失效模式。
...
| 2026-09-11 | **v1.1**：新增 §7.2.2 —— `stamp_sim_ns` 边界四路评估**已收口为路 1 + 路 4**（device 判生产侧无需拦：真机 adapter 不含该键，唯一违反路径是抄代码，属评审范畴，**路 3 拦错了位置**）。更正初稿表格把「增量 0」写成「总量 0」。备选 3-lite 留档并写死两条触发条件。 |
| 2026-09-06 | **v1.0**：新增 §7.2.1 批准后的执行归属（三方各自认领并确认）。simulator 认领候选 1/5（分提交带测试、回摘要给 ATP 核对而非由 ATP 转述），device 认领候选 7(a)。**三项互不阻塞可并行；ATP §7.3 头三项不依赖任何批准。** 两侧均报无待办，全部待 M。 |
