...
**旧版本上的替代判据**（没有 `stamp_sim_ns` 时可用，但不如显式键可靠）：
`t_src_ns == t_rx_ns` —— 退化时两者同为 `wall` 故相等；正常帧相差传输延迟，
但理论上也可能碰巧相等。
