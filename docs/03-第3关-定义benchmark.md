...
### 怎么让"预期必须红"不把整次评测拖红：`expect: fail`

问题很直接：体检用例失败是**设计如此**，但评测只有红绿两态，表达不了"这条红是预期的"。
不处理的话，这个仓会长期挂着 ❌，时间久了人就开始习惯性忽略它——**而"习惯性忽略红灯"
恰恰是这个用例本身要防的失效模式**。它的红被忽略之后，体检就没有观察者了，用例等于白设。
...
scenarios:
  - id: degraded
    description: 注入观测噪声的失效工况（预期必须失败）
    scenario: scenarios/full.yaml
    hyperparams: { noise_std: 0.2 }
    expect: fail          # ★声明：本场景设计成必须失败
```
...
| `pass` | 失败 | ❌ | 真的坏了 |
| **`fail`** | **通过** | ❌ | **判据坏了**——它已经不能拒绝坏算法了。这不是"失败"，是更值得警觉的信号 |
| 任意 | **没产出** | ❌ | **根本没跑**（数据源没给帧 / 场景装配问题）——排查方向与前两种完全不同 |

**只要所有场景都符合预期，整次评测就是绿的。** 报告里会写「N/N 场景符合预期」。

⚠ **原始事实不会被改写**：`results` 里每条 testcase 的 `passed` 照实报（`degraded` 的那两条
确实是 `false`）。改的只是**结论**的算法——否则没人知道这个场景到底跑没跑过。

> 字段定义见[附录 §3.1](07-附录-scenario-yaml-schema.md)；跨平台语义见总契约 §10 A11。
...
> ⚠ **`expect` 是场景级的，不是 testcase 级的。** 上面练习里的 `tiered` 一个场景内混了
> 必过（nominal）与必红（beyond）两类工况，所以它整体只能声明一个 `expect`——
> 有任一 testcase 失败即算该场景 `actual: fail`，表达不了"这三条各有各的期望"。
> 要分别声明，就把必红的工况**拆成独立的清单条目**：
> `cicd_test_slam` 就是这么做的——`full`（必过）与 `degraded`（必红）是两个条目，
> 共用同一份场景文件，只靠 `hyperparams` 与 `expect` 区分。

## 3.8 通关自检

- [ ] 我能说出 benchmark 的四个"固定"，以及各自缺失的后果
- [ ] 我的场景文件里，每个 testcase 都能用一句话说清它在考什么
- [ ] 我的工况里**有一个是预期必须红的**，用来体检判据，且已用 `expect: fail` 声明
