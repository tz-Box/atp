"""测试内容：nav2d 服务通路 —— server 按 world.closed_loop 装配 ClosedLoopSession，
真实 manifest（相对 scenario 路径 + launch 工作目录语义）端到端跑通。
期望输出：两 testcase 均到点 passed；comm_health 双侧留痕齐全；report.json 落盘。
"""
...
_MANIFEST = str(_ROOT / "examples" / "nav_sut.scenario.yaml")
_POLL_INTERVAL = 0.2
